...
import { Home, Car, Calendar, User, Plus, Shield } from 'lucide-react';
import { useAppStore } from '@/lib/store';
import { getVehicles, getUserBookings, getUserNotifications, DEMO_VEHICLES, DEMO_BOOKINGS, DEMO_NOTIFICATIONS } from '@/lib/firebaseService';
...
const NAV = [
  ...
  { href: '/dashboard/subscriptions', icon: Shield,   label: 'Membership' },
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const { user, authLoading, setVehicles, setBookings, setNotifications, setUnreadCount } = useAppStore();
  ...
  useEffect(() => {
    ...
    Promise.all([getVehicles(user.uid), getUserBookings(user.uid), getUserNotifications(user.uid)])
      .then(([v, b, n]) => {
  ...
  if (authLoading || !user) return (
    <div className="min-h-screen bg-main flex flex-col items-center justify-center gap-4">
      ...
      <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '11px', letterSpacing: '0.12em', color: 'var(--faint)' }}>LOADING SYSTEM</span>
    </div>
  );
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--void)' }}>
      ...
      <nav className="fixed bottom-0 inset-x-0 z-50 pb-safe"
        ...
        {/* Ember line at top */}
        <div className="h-px" style={{ background: 'linear-gradient(90deg, transparent, rgba(255,69,0,0.2), transparent)' }} />
        <div className="flex items-center justify-around max-w-lg mx-auto px-2 pt-2 pb-1">
          {NAV.map(({ href, icon: Icon, label, special }) => {
            ...
            return (
              <Link key={href} href={href}>
                <motion.div whileTap={{ scale: 0.84 }} className="flex flex-col items-center gap-0.5 px-3 py-1 min-w-[52px]">
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center transition-all"
                    style={{ background: isActive ? 'rgba(255,69,0,0.12)' : 'transparent' }}>
                    {Icon && <Icon size={18} style={{ color: isActive ? 'var(--ember)' : 'var(--steel)' }} />}
                  </div>
                  <span style={{
