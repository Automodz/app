...
export const getStatusColor = (status: string): string => ({
  ...
  in_progress:        'text-white bg-white/10',
...
/** SHA-256 hex digest via Web Crypto - used for kiosk PIN hashing */
...
export const formatInvoiceNumber = (prefix: string, year: number, seq: number): string =>
  `${prefix}-${year}-${String(seq).padStart(4, '0')}`;

/** Random URL-safe token for shareable invoice links */
export const randomToken = (length = 24): string => {
