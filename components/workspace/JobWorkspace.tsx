...
export default function JobWorkspace({ id, onBack, backLabel, onOpenBooking }: {
  id: string;
  /** drawer mode: close the drawer instead of history.back() */
  onBack?: () => void;
  backLabel?: string;
  /** drawer mode: booking-linked jobs switch the drawer instead of routing */
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (j.bookingId) {
        if (onOpenBooking) onOpenBooking(j.bookingId);
        else router.replace(`/admin/bookings/${j.bookingId}`);
        return;
      }
      setJob(j);
      setNotes(j.notes ?? '');
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto">
      <button onClick={onBack ?? (() => router.back())} className="inline-flex items-center gap-1.5 mb-4 font-mono" style={{ fontSize: 11, letterSpacing: '0.06em', color: 'var(--muted)' }}>
        <ArrowLeft size={13} /> {backLabel ?? 'ACTIVE JOBS'}
      </button>
