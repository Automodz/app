...
export default function BookingWorkspace({ id, onBack, backLabel }: {
  id: string;
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto">
      ...
      <button onClick={onBack ?? (() => router.back())} className="inline-flex items-center gap-1.5 mb-4 font-mono" style={{ fontSize: 11, letterSpacing: '0.06em', color: 'var(--muted)' }}>
        <ArrowLeft size={13} /> {backLabel ?? 'BOOKINGS'}
      </button>
        ...
        <div className="space-y-4">
          <Section title="Actions" delay={0.08}>
            <div className="space-y-2">
              ...
              <button onClick={() => router.push(`/admin/vehicles/${encodeURIComponent(booking.vehicleRegNo)}`)}
                className="flex items-center gap-2.5 w-full px-3.5 py-2.5 rounded-xl transition-colors cursor-pointer" style={{ background: 'var(--fog)', border: '1px solid var(--border-2)', color: 'var(--fg-dim)' }}>
